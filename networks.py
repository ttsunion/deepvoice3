...
def decoder(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with tf.variable_scope("decoder_prenet"):
            for i in range(hp.dec_layers):
                inputs = fc_block(inputs,
                                  ...
                                  training=training,
                                  scope="decoder_prenet_{}".format(i)) # (N, Ty, a)
        ...
        with tf.variable_scope("decoder_conv_att"):
            ...
            for i in range(hp.dec_layers):
                ...
                queries = conv_block(inputs,
                                     ...
                                     padding="CAUSAL",
                                     training=training,
                                     scope="decoder_conv_block_{}".format(i)) # (N, Ty, a)
        ...
        with tf.variable_scope("mel_logits"):
            mel_logits = fc_block(decoder_output, hp.n_mels, training=training)  # (N, Ty, n_mels)

        with tf.variable_scope("done_output"):
            done_output = fc_block(inputs, num_units = 2, training=training) # (N, Ty, 2)
        ...
        with tf.variable_scope("converter_conv"):
            for i in range(hp.converter_layers):
                outputs = conv_block(inputs,
                                     ...
                                     padding="SAME",
                                     training=training,
                                     scope="converter_conv_{}".format(i))  # (N, Ty, d)
